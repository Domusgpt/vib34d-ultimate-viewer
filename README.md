...
| Experience Shell | Demonstration UI highlighting adaptive behaviours and commercialization hooks (now composed with `createConsentPanel`, the commercialization coverage panel, the live Layout Blueprint renderer/export workflow, and the 4D projection field composer + catalog-backed scenario simulator with validation/status overlays). | `wearable-designer.html`, `src/ui/components/ConsentPanel.js`, `src/ui/adaptive/renderers/LayoutBlueprintRenderer.js`, `src/ui/adaptive/renderers/ProjectionFieldComposer.js`, `src/ui/adaptive/simulators/ProjectionScenarioSimulator.js`, `src/ui/adaptive/simulators/ProjectionScenarioCatalog.js`, `src/ui/adaptive/simulators/ProjectionScenarioValidator.js` |
...
- [Adaptive SDK Developer Handoff & Publishing Guide](DOCS/ADAPTIVE_SDK_DEVELOPER_HANDOFF_GUIDE.md) – Consolidated instructions for environment setup, commercialization workflows, QA, and release packaging.
...
- ⚠️ Publishing readiness – The new developer handoff guide documents environment setup, release packaging, and support lanes; continue deepening API contract specs per the SDK boundary proposal.
