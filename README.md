...
> **New:** The demo shell now ships with the reusable `createConsentPanel` experience, downloadable compliance exports powered by `ComplianceVaultTelemetryProvider`, the remote license attestation status feed, a projection blueprint panel driven by the new `ProjectionFieldComposer`, a projection scenario simulator for gesture/low-light/ambient crowd heuristics, and a commercialization coverage dashboard powered by the commercialization analytics stack. Use it to trial consent toggles, review the audit stream, monitor attestation outcomes, inspect projection channel plans, run what-if projection simulations, capture commercialization KPI snapshots, export BI feeds, and visualize pack coverage/adoption before embedding the SDK elsewhere.
...
| Projection Blueprint | Converts sensory/layout/design context into multi-channel projection plans for screenless surfaces. | `src/ui/adaptive/ProjectionFieldComposer.js` |
| Projection Scenario Simulator | Applies predefined or custom sensor/layout contexts to generate blueprint what-if analyses with channel metrics. | `src/ui/adaptive/ProjectionScenarioSimulator.js` |
...
| Experience Shell | Demonstration UI highlighting adaptive behaviours and commercialization/compliance hooks (now composed with `createConsentPanel`, the projection blueprint panel, and commercialization coverage utilities). | `wearable-designer.html`, `src/ui/components/ConsentPanel.js` |
