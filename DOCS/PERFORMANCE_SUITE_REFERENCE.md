...
* Tap-tempo and BPM nudge controls provide quick timing tweaks, surface visual feedback, and emit a
  `show:tempo-update` hub event so external clocks or lighting rigs can sync instantly.
...
* Added tap-tempo, fine/coarse BPM nudges, and tempo indicator badges with hub broadcasts for
