...
* A dedicated Layout & Display panel lets operators adjust interface density, pad surface scale,
  column emphasis, text sizing, and the responsive breakpoint. Layout choices persist between
  sessions (including collapsed stack states) so the suite reopens with the preferred ergonomics on
  every device.
...
* `PerformanceLayoutPanel` exposes interface density, pad surface scaling, text size, and column
  emphasis profiles alongside the default mobile panel selector. It writes changes through the layout
  storage key so ergonomics persist between reloads and across engine swaps.
