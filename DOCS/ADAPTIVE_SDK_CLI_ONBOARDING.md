...
| `init` | Scaffold a ready-to-edit config, marketplace, and sample plugin. |
...
## Scaffold a quickstart workspace

Run the `init` command to bootstrap a working directory with a ready-to-edit
configuration file, a local marketplace, and a sample greeting plugin:

```bash
node bin/adaptive-sdk-cli.js init --dir ./plugin-workspace
```

The CLI reports each file it creates (or skips) so you can review the scaffold
quickly. Re-run with `--force` to refresh files after experimentation. Additional
flags let you tailor the output:

- `--plugin <id>` — Customize the plugin id (defaults to `my-first-plugin`).
- `--name <display>` — Override the human-readable plugin name.
- `--marketplace <name>` — Pick the marketplace folder (defaults to
  `local-marketplace`).
- `--dir <path>` — Choose the root directory for generated files.
- `--force` — Overwrite existing files instead of skipping them.

Inside the target directory you will find:

- `adaptive-sdk.config.json` — An SDK-ready config that references the demo
  plugin descriptors so the CLI and runtime can load them immediately.
- `<marketplace>/.claude-plugin/marketplace.json` — A marketplace manifest that
  references the scaffolded plugin by relative path.
- `<marketplace>/<plugin>/.claude-plugin/plugin.json` — Plugin metadata compatible
  with Claude Code marketplaces.
- `<marketplace>/<plugin>/commands/hello.md` — A static command stub agents can
  customize.

Use the generated config with the CLI or runtime helpers:

```bash
node bin/adaptive-sdk-cli.js status --config ./plugin-workspace/adaptive-sdk.config.json
```

After editing the scaffold, share the marketplace directory with teammates so
their Claude Code environments install the same plugin bundle.
