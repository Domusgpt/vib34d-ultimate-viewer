...
| Experience | `createConsentPanel`, `LayoutBlueprintRenderer`, `LayoutBlueprintInsightEngine`, `LayoutBlueprintScenarioSimulator`, `LayoutBlueprintCalibrationEngine`, `LayoutBlueprintEvolutionEngine`, `createLayoutBlueprintRenderer`, `buildLayoutBlueprint`, `createLayoutBlueprintInsightEngine`, `createLayoutBlueprintScenarioSimulator`, `createLayoutBlueprintCalibrationEngine`, `createLayoutBlueprintEvolutionEngine` | Reusable consent UI plus adaptive layout blueprint visualization/export, health/insight helpers, scenario analytics, calibration intelligence, and evolution variant analysis for demos and partner toolchains. |
...
- Layout/telemetry modules expose abstract strategy/provider interfaces with Vitest coverage, and scenario analytics APIs (`generateLayoutBlueprintScenario`, scenario history accessors) ship with type definitions/tests.
