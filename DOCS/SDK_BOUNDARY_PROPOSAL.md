...
| Projection | `ProjectionFieldComposer`, `ProjectionScenarioSimulator`, `registerProjectionChannel`, `clearProjectionChannels`, `registerProjectionScenario`, `clearProjectionScenarios`, `listProjectionScenarios`, `simulateProjectionScenario`, `getProjectionBlueprint` | Translate sensory/layout/design state into projection channel blueprints, run scenario what-if analyses, and expose metrics for partner tooling. |
...
- **Projection Channels & Scenarios:** `ProjectionFieldComposer` exposes `registerChannel`/`clearChannels` while `ProjectionScenarioSimulator` enables registering default/custom heuristics and running simulations. `createAdaptiveSDK` forwards `projection`, `projectionChannels`, `replaceDefaultProjectionChannels`, `listProjectionScenarios()`, and `simulateProjectionScenario()` so downstream consumers can both inspect live blueprints and stress-test scenarios without reimplementing heuristics.【F:src/core/AdaptiveSDK.js†L1-L236】【F:types/adaptive-sdk.d.ts†L479-L620】

## Packaging Plan
1. **Phase 1:** Ship as ESM bundle + type definitions. Provide factory `createAdaptiveSDK` returning
   `{ engine, sensoryBridge, telemetry, projectionComposer, projectionScenarioSimulator, licenseManager, licenseAttestor, registerSensorSchema, registerSensorAdapter, connectSensorAdapter, disconnectSensorAdapter, testSensorAdapter, registerTelemetryProvider, registerTelemetryRequestMiddleware, clearTelemetryRequestMiddleware, registerProjectionChannel, clearProjectionChannels, registerProjectionScenario, clearProjectionScenarios, listProjectionScenarios, simulateProjectionScenario, getAdaptiveContextSnapshot, getActiveLayout, registerLicenseAttestationProfile, registerLicenseAttestationProfilePack, getLicenseAttestationProfiles, getLicenseAttestationProfile, setDefaultLicenseAttestationProfile, setLicenseAttestorFromProfile, updateTelemetryConsent, getTelemetryConsent, getTelemetryAuditTrail, getProjectionBlueprint, getLicenseCommercializationSummary, getLicenseCommercializationReporter, setLicense, validateLicense, getLicenseStatus, getLicenseHistory, getLicenseAttestationHistory, setLicenseAttestor, requestLicenseAttestation, onLicenseStatusChange, createConsentPanel }` and honoring `sensorSchemas`, `sensorAdapters`, `projection`, `projectionChannels`, `replaceDefaultProjectionChannels`, `consentOptions`, `telemetryConsent`, `license`, `licenseAttestor`, `licenseAttestationProfiles`, `licenseAttestationProfilePacks`, and `licenseAttestorProfileId`/`licenseAttestorProfileOverrides` configuration while bundling `createConsentPanel`, request signing middleware, and the compliance/commercialization storage adapters for partner reuse.
2. **Phase 2:** Publish adapters for React/Vue/Web Components. Ensure wearable demo consumes the same factory to avoid drift.
3. **Phase 3:** Release plug-in kits (Figma/Webflow) that call into the SDK boundary for previews, telemetry capture, and projection blueprint visualization.
