...
- `src/core/Engine.js` now normalizes environment options so headless/test consumers can disable canvas visualizers, DOM-bound controls, gallery/export wiring, and status messaging while the adaptive SDK surfaces a matching `environment` configuration hook.【F:src/core/Engine.js†L1-L212】【F:src/core/AdaptiveSDK.js†L1-L142】
...
6. **Headless parity gaps** – Environment gating now prevents canvas/DOM crashes in CI, but the stack lacks mock visualizers/export shims for deterministic renders, and wearable-designer modularization must expose injectable UI facades to keep headless and browser experiences aligned.【F:src/core/Engine.js†L1-L212】【F:wearable-designer.html†L1-L1568】
