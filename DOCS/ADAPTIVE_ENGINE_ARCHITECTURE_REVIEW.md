...
- `src/ui/adaptive/SensoryInputBridge.js` normalizes intent, biometric, ambient, and gesture events with configurable decay and subscription hooks.【F:src/ui/adaptive/SensoryInputBridge.js†L1-L207】
...
1. **Separation of concerns** – Sensory normalization, layout synthesis, design language mapping, and telemetry are isolated classes with single responsibilities.【F:src/ui/adaptive/SensoryInputBridge.js†L1-L207】【F:src/ui/adaptive/SpatialLayoutSynthesizer.js†L1-L124】
...
2. **Unvalidated data contracts** – Sensory bridge assumes access to browser `performance` APIs and specific payload shapes. There are no schemas, runtime guards, or fallbacks for actual hardware integrations.【F:src/ui/adaptive/SensoryInputBridge.js†L18-L103】
...
- **Short term:** The adaptive stack can power exploratory demos and concept pitches thanks to the isolated modules and compatibility with the existing visualization engine. However, the lack of schemas, SDK packaging, and automated validation makes it fragile for production licensing.
...
2. **Input Contract Hardening** – Introduce schema validation (e.g., Zod) or well-documented TypeScript interfaces, plus adapter lifecycle hooks (connect/disconnect/test) for real sensor hardware.
...
- Phase 1: Carve out SDK boundary and define input schemas.
