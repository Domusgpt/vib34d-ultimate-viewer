...
- **Callback watchers.** Reach for `sdk.watchTelemetryProviders(selector, listener, { includeExisting, once, signal, onError })` when dashboards or agent workflows prefer immediate callbacks over iterators. The helper replays existing providers by default, skips them when `includeExisting` is `false`, automatically detaches when `once` is true, and surfaces abort or listener errors through the optional `onError` hook.
- **Event targets.** Reach for `sdk.createTelemetryProviderEventTarget(selector, { includeExisting, eventName, errorEventName, disposeEventName, detail, signal })` when you need DOM-style events. The helper queues include-existing replays onto the next microtask so listeners can attach, emits configurable match/error/dispose events with metadata-rich detail payloads, and mirrors the abort/once semantics shared by the watcher and stream helpers.
- **Readable streams.** Reach for `sdk.createTelemetryProviderStream(selector, { includeExisting, signal, queuingStrategy, ReadableStream })` when you need a Web Streams `ReadableStream` to pipe provider availability into Fetch uploads, `TransformStream`s, or agent-run pipelines. The helper reuses selector metadata, replays existing providers by default, respects abort signals, and lets you supply custom stream constructors in non-DOM runtimes.
- **Collection helpers.** Reach for `sdk.collectTelemetryProviders(selector, { count, includeExisting, distinct, signal, timeoutMs })` when you need a finite batch of matches instead of an open stream. The helper resolves with projected results once the requested count is met, deduplicates providers by default, replays existing registrations, and respects the same selector metadata plus abort/timeout safeguards agents expect during orchestration.
- **Live trackers.** When you need a reusable registry instead of a terminal promise, call `sdk.trackTelemetryProviders(selector, { includeExisting, key, signal, onError })`. The helper replays existing matches (unless suppressed), emits `'add'`/`'update'` change records with metadata snapshots, exposes replay-aware subscriptions, and tears down listeners when you call `dispose()` or abort the supplied signal.
