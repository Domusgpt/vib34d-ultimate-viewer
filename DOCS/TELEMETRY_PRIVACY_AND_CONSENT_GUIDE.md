...
- **Bootstrap options.** Pass `license` or `licenseManager` into `createAdaptiveSDK` to preload license details, validators, and auto-validation behavior. The factory returns helpers—`setLicense` and `validateLicense(context)`—so partner shells can refresh entitlements or re-run attestation as part of their startup flow.【F:src/core/AdaptiveSDK.js†L1-L130】
...
4. Extend license attestation with remote revocation checks and tier synchronization so telemetry gating reflects live monetization state.
