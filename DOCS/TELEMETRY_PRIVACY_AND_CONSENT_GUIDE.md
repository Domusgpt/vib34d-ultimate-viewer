...
- **Readiness selectors.** Pair descriptor gating with `sdk.whenTelemetryProviderReady(selector, options)` to await specific provider IDs, bundles, or predicate matches (e.g., providers that expose `registerRequestMiddleware`) before enabling dependent telemetry flows or middleware.
