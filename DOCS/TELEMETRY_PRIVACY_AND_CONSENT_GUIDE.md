...
- **Remote attestation pipeline.** Provide `license.attestor` (or top-level `licenseAttestor`) configuration to instantiate a `RemoteLicenseAttestor` with attestation, revocation, and entitlement endpoints. You can also register reusable profiles via `licenseAttestationProfiles` or `registerLicenseAttestationProfile()` and apply them with `setLicenseAttestorFromProfile()` so commercialization teams ship curated partner defaults. By default the harness seeds the curated pack described in the [License Attestation Profile Catalog](LICENSE_ATTESTATION_PROFILE_CATALOG.md); disable it with `useDefaultLicenseAttestationProfiles: false` or narrow the set with `seedLicenseAttestationProfileIds`. The telemetry harness subscribes to attestation events and records `compliance.license.attestation`, `compliance.license.attestor_error`, and `system.license.attestation_scheduled` audit entries, ensuring compliance dashboards track every remote check and scheduling decision.【F:src/product/licensing/RemoteLicenseAttestor.js†L1-L253】【F:src/product/licensing/LicenseAttestationProfileRegistry.js†L1-L172】【F:src/product/licensing/attestationProfiles/defaultProfiles.js†L1-L161】【F:src/product/ProductTelemetryHarness.js†L1-L487】
...
4. Socialize the published attestation profile catalog with partner legal/compliance teams, capture certification requirements per segment, and align on SLA enforcement tooling for fail-open vs. fail-closed deployments.
