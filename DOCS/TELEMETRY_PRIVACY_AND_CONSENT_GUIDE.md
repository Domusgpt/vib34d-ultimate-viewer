...
- **Analytics exports.** `LicenseAttestationAnalytics` ingests the same audit feed and exposes per-pack counters/history via `AdaptiveInterfaceEngine#getLicenseAttestationAnalytics()` plus scoped report builders (`getLicenseAttestationAnalyticsReport()`, `exportLicenseAttestationAnalyticsReport()`) so partner dashboards and demos can surface consent-compliant metrics (validations, errors, schedules, entitlements) or download JSON snapshots without raw payload access.【F:src/product/licensing/LicenseAttestationAnalytics.js†L1-L476】【F:src/core/AdaptiveInterfaceEngine.js†L84-L106】【F:src/core/AdaptiveSDK.js†L1-L230】
