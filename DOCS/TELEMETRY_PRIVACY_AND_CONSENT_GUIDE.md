...
- [x] Surface blueprint analytics/insight history locally via `generateLayoutBlueprintSnapshot()` and the wearable insight panel without transmitting biometric-derived scores unless partners explicitly route them through consented analytics channels.【F:src/core/AdaptiveInterfaceEngine.js†L18-L150】【F:wearable-designer.html†L480-L940】
- [x] Keep blueprint scenario simulations local-only (`generateLayoutBlueprintScenario`) while anomaly tags and weighted KPIs remain unclassified; expose history through the SDK/UI without emitting telemetry until consent messaging and retention policies are finalized.【F:src/core/AdaptiveInterfaceEngine.js†L97-L212】【F:wearable-designer.html†L557-L940】【F:types/adaptive-sdk.d.ts†L730-L890】
- [x] Keep blueprint calibration runs local (`calibrateLayoutBlueprint`) until partners opt into streaming adjustment metadata; surface history through the SDK/UI with JSON exports only, tagging any future telemetry channels as `analytics` + `biometric` once consent copy/retention policies exist.【F:src/core/AdaptiveInterfaceEngine.js†L266-L360】【F:src/core/AdaptiveSDK.js†L180-L280】【F:wearable-designer.html†L703-L940】【F:types/adaptive-sdk.d.ts†L880-L1040】
- [x] Keep blueprint evolution sweeps local (`runLayoutBlueprintEvolution`) until partners opt into streaming variant analytics; expose history through the SDK/UI with JSON exports only while classifying any future telemetry uploads as `analytics` + `biometric` once consent copy and retention policies are finalized.【F:src/core/AdaptiveInterfaceEngine.js†L320-L420】【F:src/core/AdaptiveSDK.js†L200-L255】【F:wearable-designer.html†L720-L1080】【F:types/adaptive-sdk.d.ts†L734-L940】
...
5. Define how blueprint insight deltas should be classified/stored when partners request remote persistence so biometric-derived metrics remain gated behind `analytics` or dedicated classifications.
6. Establish consent copy, classifications, and retention expectations for blueprint scenario history before enabling telemetry exports or remote persistence.
7. Evaluate TypeScript adoption for `ProductTelemetryHarness` to surface classifications and consent APIs at compile time.
