...
* **Verification & Rest** – Run the diagnostics sweep, verify status badges update for pass/warn/fail
  states, confirm the activity log captures recent hub events, run both a focus session and a rest
  timer (ensure focus can auto-trigger rest when enabled), and exercise the Development Track by
  adding/removing tasks plus editing notes to confirm persistence after reload.
