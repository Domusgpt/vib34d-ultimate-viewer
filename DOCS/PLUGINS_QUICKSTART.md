...
<Tip>
  Prefer automation? Run `node bin/adaptive-sdk-cli.js init --dir ./plugin-workspace`
  to scaffold the configuration file, marketplace manifest, and greeting command
  shown below. The CLI accepts `--plugin`, `--name`, and `--marketplace`
  overrides plus `--force` for regenerating files. Continue with the manual
  steps when you want to understand each artifact in detail.
</Tip>
...
> `--demo` for the built-in sample. Run `init` to scaffold the quickstart
> workspace automatically, `check` to surface activation or metadata gaps before
> shipping, and see `DOCS/ADAPTIVE_SDK_CLI_ONBOARDING.md` for the full tour.
