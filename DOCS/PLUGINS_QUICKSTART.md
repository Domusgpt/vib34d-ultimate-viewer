...
<Tip>
  Prefer automation? Run `node bin/adaptive-sdk-cli.js init --dry-run` to preview a scaffolded marketplace, then rerun without `--dry-run` to generate the same structure with optional config files. See [Adaptive SDK CLI Onboarding](./ADAPTIVE_SDK_CLI_ONBOARDING.md#scaffolding-plugin-bundles) for a full walkthrough of the new command.
</Tip>
...
>
> **Command-line onboarding**: Use `node bin/adaptive-sdk-cli.js` for an immediate
> summary of your plugin descriptors. The CLI reports registered commands,
> agents, hooks, and MCP servers, and supports `--config` for custom bundles or
> `--demo` for the built-in sample. Run `guide` for a curated highlight reel and
> next steps, then `check` to surface activation or metadata gaps before
> shipping. See `DOCS/ADAPTIVE_SDK_CLI_ONBOARDING.md` for the full tour.

> **Programmatic onboarding**: Need the same data inside your own dashboards or
> agent workflows? Call `sdk.buildPluginOnboardingGuide()` after initializing the
> Adaptive SDK to obtain the overview, recommendations, and plugin highlights as
> JSON. Pair it with the CLI output to keep your MVP release notes and runtime
> telemetry aligned.
