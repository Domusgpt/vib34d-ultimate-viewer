...
- **`styles/performance.css`** delivers the stage-ready UI: gradient panels, glowing buttons,
  ...
  tweaks.
- **New in this revision:** the suite shell now mounts a mobile navigation bar, collapsible
  stacks with status feedback, and layout defaults (breakpoints + active panel) exposed via
  `PerformanceConfig.layout`. Telemetry styling picked up energy badges, pulse highlights,
  and history formatting so diagnostics match the refreshed aesthetic.
...
- **`DOCS/PERFORMANCE_SUITE_VISUAL_TESTING.md`** outlines manual viewport sweeps, module
  collapse/expand checks, and theming steps so visual regressions are easy to spot during QA.
