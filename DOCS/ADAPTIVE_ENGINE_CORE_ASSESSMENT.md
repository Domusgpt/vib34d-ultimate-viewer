...
4. **Commercialization Hardening** – Consent layer and remote export retention/encryption hooks landed, but telemetry still needs signed provider integrations and monetization metadata requires persistence/export surfaces for partner platforms.
...
1. Implement Playwright smoke specs tagged per the automation plan to validate consent/compliance flows end-to-end.
2. Formalize telemetry request-signing middleware and sample provider configurations under the SDK boundary proposal.
3. Package encryption template examples (KMS wrapper, envelope metadata) for partners integrating remote storage adapters.
