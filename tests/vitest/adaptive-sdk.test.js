...
describe('createAdaptiveSDK', () => {
  ...
  describe('plugin orchestration', () => {
    ...
    it('creates a default plugin scaffold plan with marketplace and config files', () => {
      const sdk = createHeadlessAdaptiveSDK();

      const plan = sdk.createPluginScaffoldPlan();

      expect(plan.marketplace?.name).toBe('test-marketplace');
      expect(plan.plugin.id).toBe('my-first-plugin');
      expect(plan.summary.includeMarketplace).toBe(true);
      expect(plan.summary.includeConfig).toBe(true);
      expect(plan.summary.includeCommand).toBe(true);
      expect(plan.directories.some((dir) => dir.path === 'test-marketplace')).toBe(true);

      const pluginManifestFile = plan.files.find((file) => file.path.endsWith('.claude-plugin/plugin.json'));
      expect(pluginManifestFile).toBeTruthy();
      const manifest = JSON.parse(pluginManifestFile?.content ?? '{}');
      expect(manifest.name).toBe('My First Plugin');
      expect(manifest.author.name).toBe('Your Name');

      const configFile = plan.files.find((file) => file.path === 'adaptive-sdk.config.json');
      expect(configFile).toBeTruthy();
      const config = JSON.parse(configFile?.content ?? '{}');
      expect(config.plugins).toHaveLength(1);
      expect(config.plugins[0].commands[0].response).toContain('Welcome');
      expect(plan.instructions[0]).toContain('node bin/adaptive-sdk-cli.js status');
    });

    it('supports plugin-only scaffold plans with customized command metadata', () => {
      const sdk = createHeadlessAdaptiveSDK();

      const plan = sdk.createPluginScaffoldPlan({
        includeMarketplace: false,
        includeConfig: false,
        pluginId: 'team-helper',
        pluginDisplayName: 'Team Helper',
        pluginTags: ['team', 'helper'],
        commandName: 'assist',
        commandDescription: 'Assist teammates quickly.'
      });

      expect(plan.marketplace).toBeNull();
      expect(plan.summary.includeMarketplace).toBe(false);
      expect(plan.config).toBeNull();
      expect(plan.summary.includeConfig).toBe(false);
      expect(plan.plugin.id).toBe('team-helper');
      expect(plan.plugin.name).toBe('Team Helper');

      const commandFile = plan.files.find((file) => file.path.endsWith('assist.md'));
      expect(commandFile?.content).toContain('# Assist');
      expect(commandFile?.content).toContain('Assist teammates quickly.');
      expect(plan.instructions[0]).toContain('Add the generated plugin descriptors');
    });
