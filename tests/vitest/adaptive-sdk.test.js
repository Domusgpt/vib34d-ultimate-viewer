...
describe('createAdaptiveSDK', () => {
  ...
  it('registers plugins from config and invokes commands, agents, and hooks', async () => {
    const sdk = createHeadlessAdaptiveSDK({
      plugins: [
        {
          ...
          commands: {
            hello: {
              ...
              handler: ({ payload }) => `Hello ${payload?.name ?? 'world'}`
            }
          },
          agents: {
            concierge: {
          ...
          hooks: {
            'demo:init': ({ payload }) => {
              if (payload && Array.isArray(payload.events)) {
                payload.events.push('greetings-plugin:init');
              }
              return payload;
            }
          }
    ...
    expect(sdk.listPlugins().map((plugin) => plugin.manifest.name)).toEqual([
      'greetings-plugin'
    ]);

    const commandResult = await sdk.invokePluginCommand('hello', { name: 'Sky' });
    expect(commandResult).toBe('Hello Sky');

    const concierge = await sdk.createPluginAgent('concierge', { visitor: 'Kai' });
    expect(typeof concierge.greet).toBe('function');
    expect(concierge.greet()).toBe('Welcome Kai');

    const hookPayload = { events: [] };
    const hookResults = await sdk.emitPluginHook('demo:init', hookPayload);
    expect(hookResults).toHaveLength(1);
    expect(hookPayload.events).toEqual(['greetings-plugin:init']);
  });

  it('installs plugins from marketplaces and resolves watchers', async () => {
    const sdk = createHeadlessAdaptiveSDK();

    await sdk.whenPluginsReady();

    const pluginEvents = [];
    const stopWatching = sdk.watchPlugins((event) => {
      pluginEvents.push(event);
    });

    const marketplace = sdk.registerPluginMarketplace({
    ...
    expect(marketplace.plugins.has('metrics-plugin')).toBe(true);

    const waitForPlugin = sdk.whenPluginRegistered('metrics-plugin');

    const pluginRecord = await sdk.installPluginFromMarketplace(
      'dev-marketplace',
      'metrics-plugin'
    );

    const resolvedPlugin = await waitForPlugin;

    expect(pluginRecord.manifest.name).toBe('metrics-plugin');
    expect(resolvedPlugin.manifest.name).toBe('metrics-plugin');
    expect(
      pluginEvents.some((event) => event.plugin.manifest.name === 'metrics-plugin')
    ).toBe(true);

    const pingResult = await sdk.invokePluginCommand('ping');
    expect(pingResult).toBe('metrics-plugin');

    stopWatching();

    const removed = sdk.unregisterPlugin('metrics-plugin', { reason: 'test-cleanup' });
