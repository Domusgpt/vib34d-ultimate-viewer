...
export function createAdaptiveSDK(config = {}) {
    ...
    function createTelemetryProviderStream(selector, options = {}) {
        ...
        const iterable = streamTelemetryProviders(selector, {
            includeExisting,
        ...
        const abortUpstream = (reason) => {
            ...
            const failure = toError(reason, 'Telemetry provider readable stream aborted.');
        ...
        return new streamConstructor({
            async start() {
                ...
                if (options.signal) {
                    if (options.signal.aborted) {
                        abortUpstream(options.signal.reason);
                        return;
                    }
