...
export function createAdaptiveSDK(config = {}) {
    ...
    const diagnosePlugins = (options = {}) => {
        ...
        for (const record of pluginRecords.values()) {
            ...
            const snapshot = createPluginSnapshot(record);
            const issues = [];

            const pushIssue = ({ severity, code, message, suggestion, details }) => {
                issues.push({
            ...
            if (snapshot.status === 'error') {
                pushIssue({
                    severity: 'error',
                    code: 'plugin-status-error',
                    message: 'Plugin failed during activation or deactivation.',
                    suggestion: 'Review the plugin setup hooks or inspect CLI logs for thrown errors.',
                    details: { status: snapshot.status }
                });
            } else if (snapshot.status !== 'active') {
                pushIssue({
                    severity: 'warning',
                    code: 'plugin-inactive',
                    message: `Plugin is registered with status "${snapshot.status}" instead of "active".`,
                    suggestion: 'Call `sdk.activatePlugin(id)` or mark the descriptor with explicit activation intent.'
                });
            }
            ...
            if (!snapshot.description || !snapshot.description.trim()) {
                pushIssue({
                    severity: 'info',
                    code: 'missing-plugin-description',
                    message: 'Plugin is missing a description for onboarding output.',
                    suggestion: 'Add a `description` field so agents understand the plugin purpose.'
                });
            }
            ...
            if (!snapshot.tags || snapshot.tags.length === 0) {
                pushIssue({
                    severity: 'info',
                    code: 'missing-plugin-tags',
                    message: 'Plugin does not define any tags.',
                    suggestion: 'Provide `tags` to improve marketplace discovery and CLI filtering.'
                });
            }

            const capabilities = new Set(snapshot.capabilities || []);
            const hasCommands = snapshot.commands.length > 0;
            const hasServers = snapshot.servers.length > 0;
            const hasHooks = Array.isArray(snapshot.hooks) && snapshot.hooks.length > 0;
            const hasAgents = snapshot.agents.length > 0;

            if (hasCommands && !capabilities.has('commands')) {
                pushIssue({
                    severity: 'warning',
                    code: 'missing-commands-capability',
                    message: 'Plugin registers commands without advertising the `commands` capability.',
                    suggestion: 'Add "commands" to the plugin `capabilities` array so tooling can discover handlers.'
                });
            }

            if (hasAgents && !capabilities.has('agents')) {
                pushIssue({
                    severity: 'info',
                    code: 'missing-agents-capability',
                    message: 'Plugin registers agents but omits the `agents` capability.',
                    suggestion: 'Add "agents" to the `capabilities` array to aid discovery.'
                });
            }

            if (hasHooks && !capabilities.has('hooks')) {
                pushIssue({
                    severity: 'info',
                    code: 'missing-hooks-capability',
                    message: 'Plugin registers hooks without the `hooks` capability tag.',
                    suggestion: 'List "hooks" in the `capabilities` array for accurate metadata.'
                });
            }

            if (hasServers && !capabilities.has('mcp')) {
                pushIssue({
                    severity: 'info',
                    code: 'missing-mcp-capability',
                    message: 'Plugin exposes MCP servers without advertising an MCP capability.',
                    suggestion: 'Add a capability such as "mcp" or "servers" so integrators can flag connection workflows.'
                });
            }

            for (const command of snapshot.commands) {
                if (!command.description || !String(command.description).trim()) {
                    pushIssue({
                        severity: 'info',
                        code: 'command-missing-description',
                        message: `Command "${command.name}" is missing a description.`,
                        suggestion: 'Add a command description so CLI listings remain actionable.',
            ...
            for (const agent of snapshot.agents) {
                if (!agent.description || !String(agent.description).trim()) {
                    pushIssue({
                        severity: 'info',
                        code: 'agent-missing-description',
                        message: `Agent "${agent.name}" does not define a description.`,
                        suggestion: 'Document the agent behaviour so operators can route tasks appropriately.',
            ...
            for (const server of snapshot.servers) {
                if (!server.description || !String(server.description).trim()) {
                    pushIssue({
                        severity: 'info',
                        code: 'server-missing-description',
                        message: `MCP server "${server.id}" is missing a description.`,
                        suggestion: 'Explain the MCP entry point so agents know what capabilities it exposes.',
                        details: { server: server.id }
                    });
                }
            }

            const highestIssue = issues.reduce((current, issue) => {
                const value = severityRank.get(issue.severity) || 0;
                if (!current) {
                    return issue.severity;
                }
                const currentValue = severityRank.get(current) || 0;
                return value > currentValue ? issue.severity : current;
            }, null);

            diagnostics.push({
                plugin: snapshot,
                issues,
                severity: highestIssue
            });
        }
    ...
    const buildPluginOnboardingGuide = (options = {}) => {
        const includeInactive = options.includeInactive !== false;
        ...
        const commandLimit = sanitizeLimit(options.commandLimit);
        const agentLimit = sanitizeLimit(options.agentLimit);
        const hookLimit = sanitizeLimit(options.hookLimit);
        const serverLimit = sanitizeLimit(options.serverLimit);
        const issueLimit = sanitizeLimit(options.issueLimit);
        const suggestionLimit = sanitizeLimit(options.suggestionLimit);
        const highlightLimit = sanitizeLimit(options.maxHighlights);
        ...
        const commandEntries = listPluginCommands({ includeInactive });
        const agentEntries = listPluginAgents({ includeInactive });
        const hookEntries = listPluginHooks({ includeInactive });
        const serverEntries = listPluginServers({ includeInactive });

        const commandsByPlugin = new Map();
        for (const entry of commandEntries) {
            const bucket = commandsByPlugin.get(entry.pluginId) || [];
            bucket.push({
                pluginId: entry.pluginId,
                name: entry.name,
            ...
            commandsByPlugin.set(entry.pluginId, bucket);
        }

        const agentsByPlugin = new Map();
        for (const entry of agentEntries) {
            const bucket = agentsByPlugin.get(entry.pluginId) || [];
            bucket.push({
                pluginId: entry.pluginId,
                name: entry.name,
                description: entry.description ?? null,
            ...
            agentsByPlugin.set(entry.pluginId, bucket);
        }

        const hooksByPlugin = new Map();
        for (const entry of hookEntries) {
            const bucket = hooksByPlugin.get(entry.pluginId) || [];
            bucket.push({
                pluginId: entry.pluginId,
                event: entry.event,
                count: entry.count
            });
            hooksByPlugin.set(entry.pluginId, bucket);
        }

        const serversByPlugin = new Map();
        for (const entry of serverEntries) {
            const bucket = serversByPlugin.get(entry.pluginId) || [];
            bucket.push({
                pluginId: entry.pluginId,
                id: entry.id,
                name: entry.name,
            ...
            serversByPlugin.set(entry.pluginId, bucket);
        }

        const diagnostics = options.includeDiagnostics === false
            ? null
            : diagnosePlugins({ includeInactive });

        const diagnosticsByPlugin = new Map();
        const diagnosticTotals = { error: 0, warning: 0, info: 0 };
        const severityRank = new Map([
            ['error', 3],
            ['warning', 2],
            ['info', 1],
        ...
        if (diagnostics) {
            for (const entry of diagnostics) {
                diagnosticsByPlugin.set(entry.plugin.id, entry);
                for (const issue of entry.issues) {
                    if (issue.severity === 'error') {
                        diagnosticTotals.error += 1;
                    } else if (issue.severity === 'warning') {
                        diagnosticTotals.warning += 1;
                    } else {
                        diagnosticTotals.info += 1;
                    }
                }
            }
        }

        const pluginHighlights = pluginSnapshots
            .map(snapshot => {
                const commandList = commandsByPlugin.get(snapshot.id) || [];
                const agentList = agentsByPlugin.get(snapshot.id) || [];
                const hookList = hooksByPlugin.get(snapshot.id) || [];
                const serverList = serversByPlugin.get(snapshot.id) || [];

                const diagnosticEntry = diagnosticsByPlugin.get(snapshot.id);
                const issues = diagnosticEntry?.issues ?? [];
                const limitedIssues = take(issues.map(issue => ({
                    code: issue.code,
                ...
                const suggestionPool = issues.filter(issue => issue.suggestion);
                const limitedSuggestions = take(suggestionPool.map(issue => ({
                    code: issue.code,
                    severity: issue.severity,
                ...
                return {
                    ...
                    commands: take(commandList, commandLimit),
                    agents: take(agentList, agentLimit),
                    hooks: take(hookList, hookLimit),
                    servers: take(serverList, serverLimit),
                    issues: limitedIssues,
                    issueCount: issues.length,
                    suggestions: limitedSuggestions,
                    suggestionCount: suggestionPool.length,
                    severity: diagnosticEntry?.severity ?? 'none'
                };
            })
            .sort((a, b) => {
                ...
                if (a.plugin.status === 'active' && b.plugin.status !== 'active') {
                    return -1;
                }
                if (b.plugin.status === 'active' && a.plugin.status !== 'active') {
                    return 1;
                }
                return String(a.plugin.id).localeCompare(String(b.plugin.id));
            });

        const limitedHighlights = highlightLimit === undefined
            ? pluginHighlights
            : take(pluginHighlights, highlightLimit === 0 ? 0 : highlightLimit);
        ...
        if (diagnostics) {
            for (const entry of diagnostics) {
                for (const issue of entry.issues) {
                    ...
                    const key = `${entry.plugin.id}:${issue.code}:${issue.suggestion}`;
        ...
        const topCommandNames = [];
        for (const highlight of pluginHighlights) {
            for (const command of highlight.commands) {
                if (!command.name) {
                    continue;
                }
                if (!topCommandNames.includes(command.name)) {
                    topCommandNames.push(command.name);
                }
                if (topCommandNames.length >= 3) {
                    break;
                }
            }
            if (topCommandNames.length >= 3) {
        ...
        const hasServerHighlights = pluginHighlights.some(entry => entry.counts.servers > 0);
        const hasAgentHighlights = pluginHighlights.some(entry => entry.counts.agents > 0);
        const hasHookHighlights = pluginHighlights.some(entry => entry.counts.hooks > 0);

        const nextStepsSet = new Set([
            'Share this summary with your agents so they know which plugins are ready.',
            'Use `node bin/adaptive-sdk-cli.js diagnostics --json` to export health reports for CI gates.'
        ]);

        const activeCount = statusCounts.active ?? 0;
        if (activeCount === 0 && pluginSnapshots.length > 0) {
            nextStepsSet.add('Activate at least one plugin with `sdk.activatePlugin(id)` or mark descriptors as active to unlock functionality.');
        }

        if (commandEntries.length === 0) {
            nextStepsSet.add('Define command descriptors so agents have slash commands to invoke.');
        } else if (topCommandNames.length > 0) {
            nextStepsSet.add(`Introduce agents to commands like ${topCommandNames.map(name => `/${name}`).join(', ')}.`);
        }

        if (recommendations.some(entry => entry.severity === 'error')) {
            nextStepsSet.add('Resolve blocking diagnostic errors before distributing the plugin bundle.');
        }
        if (recommendations.some(entry => entry.severity === 'warning')) {
            nextStepsSet.add('Address diagnostic warnings to improve metadata completeness and activation.');
        }

        if (hasServerHighlights) {
            nextStepsSet.add('Test MCP server integrations with `sdk.connectPluginServer(...)` or via the CLI `servers list` view.');
        }
        if (hasAgentHighlights) {
            nextStepsSet.add('Document how to invoke helper agents via `sdk.invokePluginAgent(id, ...)` for downstream workflows.');
        }
        if (hasHookHighlights) {
            nextStepsSet.add('Review hook coverage and bind automation to the events surfaced in the hook highlights.');
        }

        const overview = {
            pluginTotal: pluginSnapshots.length,
            activePlugins: activeCount,
            inactivePlugins: pluginSnapshots.length - activeCount,
    ...
    const createPluginScaffoldPlan = (options = {}) => {
        const joinSegments = (...segments) => {
            ...
            for (const segment of segments) {
                ...
                cleaned.push(stringValue.replace(/^\/+|\/+$/g, ''));
            }
            if (cleaned.length === 0) {
                return '';
            }
        ...
        const toSlug = (value, fallback) => {
            ...
            const normalized = String(source)
                ...
                .replace(/^-+|-+$/g, '');
            if (!normalized) {
                return fallback ? toSlug(fallback) : '';
            }
            return normalized;
        };
        ...
        const marketplaceName = options.marketplaceName || 'test-marketplace';
        const marketplaceSlug = toSlug(options.marketplaceId || marketplaceName || 'marketplace');
        const marketplaceDirectory = joinSegments(
            options.marketplaceDirectory || options.marketplaceDir || (includeMarketplace ? marketplaceSlug : '' )
        );
        const marketplaceOwner = options.marketplaceOwner || options.ownerName || options.authorName || 'Test User';

        const pluginId = toSlug(options.pluginId || options.pluginName || 'my-first-plugin', 'my-first-plugin');
        const pluginDirectory = joinSegments(
            includeMarketplace ? marketplaceDirectory : '',
            options.pluginDirectory || options.pluginDir || pluginId
        );
        const pluginDisplayName = options.pluginDisplayName || options.pluginTitle || toTitle(pluginId, 'My First Plugin');
        const pluginVersion = options.pluginVersion || options.version || '0.1.0';
        const pluginDescription = options.pluginDescription || 'A simple greeting plugin to learn the basics';
        const pluginTags = parseList(options.pluginTags) || ['demo', 'onboarding'];
        const pluginCapabilities = parseList(options.pluginCapabilities) || (includeCommand ? ['commands'] : []);
        const authorName = options.authorName || 'Your Name';

        const commandName = includeCommand ? toSlug(options.commandName || 'hello', 'hello') : null;
        const commandHeading = options.commandHeading || options.commandTitle || toTitle(commandName, 'Hello Command');
        const commandDescription = options.commandDescription || 'Greet the user with a personalized message';
        const commandBody = options.commandBody || 'Greet the user warmly and ask how you can help them today. Make the greeting personal and encouraging.';
        const commandResponse = options.commandResponse || `👋 Welcome to ${pluginDisplayName}! Customize commands/${commandName ?? 'hello'}.md to teach agents how to introduce your plugin.`;

        const configFileName = options.configFileName || 'adaptive-sdk.config.json';

        const directories = new Map();
        const addDirectory = (pathName, description) => {
            const normalized = joinSegments(pathName);
            if (!normalized) {
                return;
            }
            if (!directories.has(normalized)) {
                directories.set(normalized, { path: normalized, description: description || null });
            }
        };

        const files = [];
        const addFile = (pathName, description, content) => {
            const normalized = joinSegments(pathName);
            if (!normalized) {
                return;
            }
        ...
        if (includeMarketplace && marketplaceDirectory) {
            addDirectory(marketplaceDirectory, 'Marketplace root for local development.');
            addDirectory(joinSegments(marketplaceDirectory, '.claude-plugin'), 'Marketplace manifest directory.');
        }

        addDirectory(pluginDirectory, 'Plugin root directory.');
        addDirectory(joinSegments(pluginDirectory, '.claude-plugin'), 'Plugin manifest directory.');
        if (includeCommand) {
            addDirectory(joinSegments(pluginDirectory, 'commands'), 'Slash command definitions.');
        }
                ...
                plugins: [
                    {
                        ...
                        source: joinSegments('./', options.pluginDirectory || options.pluginDir || pluginId),
                        description: options.marketplacePluginDescription || pluginDescription
                    }
                ]
            };
            addFile(
                joinSegments(marketplaceDirectory, '.claude-plugin', 'marketplace.json'),
                'Marketplace manifest describing the local plugin.',
        ...
        addFile(
            joinSegments(pluginDirectory, '.claude-plugin', 'plugin.json'),
            'Plugin metadata manifest.',
            JSON.stringify(pluginManifest, null, 2) + '\n'
        );

        if (includeCommand && commandName) {
            const commandContent = `---\ndescription: ${commandDescription}\n---\n\n# ${commandHeading}\n\n${commandBody}\n`;
            addFile(
                joinSegments(pluginDirectory, 'commands', `${commandName}.md`),
                'Sample slash command definition.',
                commandContent
            );
        }
        ...
        if (includeConfig) {
            const pluginEntry = {
                ...
                description: pluginDescription,
                version: pluginVersion,
                tags: pluginTags.length ? [...new Set(pluginTags)] : undefined,
                capabilities: pluginCapabilities.length ? [...new Set(pluginCapabilities)] : undefined
            };
            ...
            addFile(
                configFileName,
                'Adaptive SDK configuration for CLI onboarding.',
        ...
        const instructions = [];
        if (includeConfig) {
            instructions.push(`Run \`node bin/adaptive-sdk-cli.js status --config ${configFileName}\` to verify plugin registration.`);
        } else {
            instructions.push('Add the generated plugin descriptors to your Adaptive SDK config and run `node bin/adaptive-sdk-cli.js status --config <path>` to verify registration.');
        }
        if (includeMarketplace) {
            const marketplacePath = marketplaceDirectory ? `./${marketplaceDirectory}` : '.';
            instructions.push(
                `Install the marketplace in Claude Code with \`/plugin marketplace add ${marketplacePath}\` and \`/plugin install ${pluginId}@${marketplaceName}\`.`
            );
        } else {
            instructions.push('Install your plugin files into an existing marketplace or share descriptors directly with your team.');
        }
        instructions.push('Share commands and docs with your team so agents know how to engage the plugin.');
        ...
        return {
            ...
            command: includeCommand && commandName
                ? {
                    name: commandName,
    ...
    const sdk = {
        ...
        listPluginServers,
        diagnosePlugins,
        buildPluginOnboardingGuide,
        createPluginScaffoldPlan,
