...
export class VIB34DIntegratedEngine {
    constructor() {
        ...
        this.performanceSuiteConfig = null;
        this.performanceConfigListener = null;
    ...
    init() {
        ...
        try {
            ...
            this.bindPerformanceSuiteConfigEvents();
    ...
    initializePerformanceSuite() {
        ...
        try {
            ...
            const config = this.resolvePerformanceSuiteConfig();
    ...
    resolvePerformanceSuiteConfig() {
        if (this.performanceSuiteConfig) {
            return this.performanceSuiteConfig;
        }
        if (typeof window !== 'undefined' && window.performanceSuiteConfig) {
            return window.performanceSuiteConfig;
        }
        return {};
    }

    setPerformanceSuiteConfig(config, options = {}) {
        this.performanceSuiteConfig = config;
    ...
    bindPerformanceSuiteConfigEvents() {
        if (typeof window === 'undefined' || !window.addEventListener) return;

        if (this.performanceConfigListener) {
            window.removeEventListener('vib34d:performance-config', this.performanceConfigListener);
        }

        this.performanceConfigListener = (event) => {
            const detail = event?.detail;
            if (!detail) return;

            const payload = detail.config || detail;
            const shouldReinitialize = detail && typeof detail === 'object' && 'reinitialize' in detail
                ? detail.reinitialize
                : true;

            if (payload && typeof payload === 'object') {
                this.setPerformanceSuiteConfig(payload, { reinitialize: shouldReinitialize });
            }
        };

        window.addEventListener('vib34d:performance-config', this.performanceConfigListener);
    }

    getPerformanceSuiteSnapshot() {
        return this.performanceSuite?.getSnapshot?.() || null;
    }
    ...
    applyAudioReactivityGrid(audioData) {
        ...
        if (settings.bands) {
            ...
            const processed = this.prepareSmoothedAudio(audioData, settings.smoothing);
    ...
    prepareSmoothedAudio(audioData, smoothing = 0.35) {
        const smoothed = {};
        const bands = ['bass', 'mid', 'high', 'energy', 'rhythm', 'melody'];

        bands.forEach(band => {
            const value = typeof audioData?.[band] === 'number' ? audioData[band] : null;
            if (value === null) return;

            const previous = this.audioSmoothingState[band] ?? value;
            const blend = this.clamp01(smoothing);
            const smoothedValue = previous * blend + value * (1 - blend);
