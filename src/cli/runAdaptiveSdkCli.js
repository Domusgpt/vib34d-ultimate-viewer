...
function renderWelcome(stdout) {
    ...
    stdout.write('  • Run `node bin/adaptive-sdk-cli.js init --dir ./plugin-workspace` to scaffold a quickstart workspace.\n');
...
async function writeScaffoldFile({
    ...
    if (exists && !force) {
        stdout.write(`⚠️  Skipped ${label} (${formatRelative(root, filePath)} already exists). Use --force to overwrite.\n`);
        return false;
    }
    ...
    stdout.write(`${icon}${verb} ${label} (${formatRelative(root, filePath)})\n`);
    return true;
}

async function handleInit({ stdout, values, flags, env }) {
    const cwd = env?.PWD ? path.resolve(env.PWD) : process.cwd();
    const targetDir = values.get('dir') || values.get('d') || '.';
    const pluginIdInput = values.get('plugin') || values.get('p') || 'my-first-plugin';
    const marketplaceNameInput = values.get('marketplace') || values.get('m') || 'local-marketplace';
    const pluginNameInput = values.get('name') || values.get('n');
    const force = flags.has('force') || flags.has('f');

    const resolvedRoot = path.resolve(cwd, targetDir);
    const pluginId = pluginIdInput.trim() || 'my-first-plugin';
    const marketplaceName = marketplaceNameInput.trim() || 'local-marketplace';
    const pluginName = (pluginNameInput || toTitleCase(pluginId)) || 'My First Plugin';

    const configPath = path.join(resolvedRoot, 'adaptive-sdk.config.json');
    const marketplaceRoot = path.join(resolvedRoot, marketplaceName);
    const marketplaceManifestPath = path.join(marketplaceRoot, '.claude-plugin', 'marketplace.json');
    const pluginRoot = path.join(marketplaceRoot, pluginId);
    const pluginManifestPath = path.join(pluginRoot, '.claude-plugin', 'plugin.json');
    const commandMarkdownPath = path.join(pluginRoot, 'commands', 'hello.md');
        ...
        plugins: [
            {
                ...
                name: pluginName,
                version: '1.0.0',
                description: 'A simple greeting plugin scaffolded by the Adaptive SDK CLI.',
                tags: ['demo', 'scaffold'],
                ...
                commands: [
                    {
                        ...
                        response: '👋 Hello from the Adaptive SDK scaffold! Let us know how we can help.'
                    }
                ]
            }
        ]
        ...
        plugins: [
            {
                name: pluginId,
                source: `./${pluginId}`,
                description: 'Plugin scaffold generated by the Adaptive SDK CLI'
            }
        ]
    }, null, 2) + '\n';

    const pluginManifestContent = JSON.stringify({
        name: pluginId,
        description: 'A simple greeting plugin scaffolded by the Adaptive SDK CLI.',
        version: '1.0.0',
        author: {
    ...
    const commandMarkdownContent = `---\ndescription: Greet the user with a personalized message\n---\n\n# Hello Command\n\nGreet the user warmly and ask how you can help them today. Make the greeting personal and encouraging.\n`;

    stdout.write('\nAdaptive SDK CLI — Quickstart Scaffold\n');
    stdout.write('======================================\n');
    stdout.write(`Target directory : ${resolvedRoot}\n`);
    stdout.write(`Plugin id        : ${pluginId}\n`);
    stdout.write(`Marketplace name : ${marketplaceName}\n`);
    stdout.write(`Force overwrite  : ${force ? 'yes' : 'no'}\n\n`);

    let writes = 0;
    writes += (await writeScaffoldFile({
        root: resolvedRoot,
        filePath: configPath,
        content: configContent,
        label: 'Adaptive SDK config',
        force,
        stdout
    })) ? 1 : 0;
    ...
    writes += (await writeScaffoldFile({
        root: resolvedRoot,
        filePath: commandMarkdownPath,
        content: commandMarkdownContent,
    ...
    stdout.write('\nNext steps:\n');
    stdout.write(`  • Edit ${formatRelative(resolvedRoot, configPath)} to describe your real plugins.\n`);
    stdout.write(`  • Run \`node bin/adaptive-sdk-cli.js status --config ${formatRelative(resolvedRoot, configPath)}\` to verify descriptors.\n`);
    stdout.write(`  • Add the marketplace (${formatRelative(resolvedRoot, marketplaceRoot)}) to Claude Code or your agent tooling.\n`);
    stdout.write('  • Re-run with `--force` to refresh the scaffold after local edits.\n\n');

    stdout.write(`Scaffold complete — ${writes} file${writes === 1 ? '' : 's'} written.\n\n`);
    ...
    try {
        switch (command) {
            ...
            case 'help':
                ...
                return 0;
            case 'init':
                return await handleInit({ stdout, values, flags, env });
            case 'status':
