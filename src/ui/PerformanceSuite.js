...
export class PerformanceSuite {
    ...
    normalizeLayoutConfig(layout = {}) {
        ...
        const normalized = { ...defaults, ...(layout || {}) };
        normalized.mobileBreakpoint = Number(normalized.mobileBreakpoint) || defaults.mobileBreakpoint;
        normalized.defaultPanel = normalized.defaultPanel || defaults.defaultPanel;
        if (!Array.isArray(normalized.collapsedStacks)) {
            normalized.collapsedStacks = defaults.collapsedStacks.slice();
        }
        return normalized;
    }
            ...
            <div class="performance-suite__columns">
                <section class="performance-suite__column performance-suite__column--pads" data-panel="pads">
                    <article class="performance-suite__stack" data-stack="touchpads">
                        ...
                        <div class="performance-suite__stack-body" id="performance-stack-body-touchpads" data-role="pads"></div>
                    </article>
    ...
    setupResponsiveBehavior() {
        ...
        if (typeof window !== 'undefined') {
            this.onResize = () => {
                ...
                const mode = viewportWidth <= this.layoutConfig.mobileBreakpoint ? 'mobile' : 'desktop';
    ...
    setupCollapsibleStacks() {
        ...
        stacks.forEach((stack) => {
            ...
            const handler = () => {
                ...
                this.setStackCollapsed(stack, button, body, !collapsed);
                const label = this.stackLabels[target] || 'section';
                this.updateStatus(`${label} ${collapsed ? 'expanded' : 'collapsed'}`);
            };
