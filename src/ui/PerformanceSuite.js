...
export class PerformanceSuite {
    ...
    initModules() {
        ...
        this.subscriptions.push(this.hub.on('show:tempo-update', ({ tempo, source, beatsPerBar } = {}) => {
            if (!Number.isFinite(tempo)) {
                return;
            }
            if (source === 'beats' && Number.isFinite(beatsPerBar)) {
                this.updateStatus(`Beats / bar: ${Math.round(beatsPerBar)}`);
                return;
            }
            const label = source === 'tap'
                ? 'Tap tempo'
                : source === 'nudge'
                    ? 'Tempo nudged'
                    : source === 'state-apply'
                        ? 'Tempo restored'
                        : 'Tempo updated';
            this.updateStatus(`${label}: ${Math.round(tempo)} BPM`);
        }));
    }
