...
function normalizeThemeState(themeState = null) {
    if (!themeState) {
        return { paletteId: 'system', overrides: null };
    }

    const normalized = { paletteId: themeState.paletteId || 'system' };
    if (themeState.overrides) {
        normalized.overrides = {
            accent: themeState.overrides.accent || undefined,
            highlightAlpha: typeof themeState.overrides.highlightAlpha === 'number'
                ? themeState.overrides.highlightAlpha
                : undefined,
            glowStrength: typeof themeState.overrides.glowStrength === 'number'
                ? themeState.overrides.glowStrength
                : undefined
        };
    } else {
        normalized.overrides = null;
    }
    return normalized;
}
