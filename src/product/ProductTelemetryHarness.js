...
import { LicenseAttestationAnalytics } from './licensing/LicenseAttestationAnalytics.js';
...
export class ProductTelemetryHarness {
    constructor(options = {}) {
        ...
        this.licenseAttestationAnalytics = new LicenseAttestationAnalytics(options.licenseAnalytics || {});
        this.activeLicenseAttestationProfileId = null;
    ...
    registerLicenseAttestationProfile(profileOrId, maybeProfile, context = {}) {
        ...
        this.licenseAttestationAnalytics.registerProfile(profile, context);
    ...
    registerLicenseAttestationProfilePack(packOrId, options = {}) {
        ...
        this.licenseAttestationAnalytics.registerPack(pack);
        ...
        for (const profile of pack.profiles) {
            const result = this.registerLicenseAttestationProfile(profile, undefined, {
                ...
                isDefault: pack.defaultProfileId === profile.id
            });
        ...
        if (pack.defaultProfileId && options.applyDefault !== false) {
            ...
            this.licenseAttestationAnalytics.setPackDefault(pack.id, pack.defaultProfileId);
    ...
    setDefaultLicenseAttestationProfile(id) {
        ...
        this.licenseAttestationAnalytics.setDefaultProfile(id);
    ...
    setLicenseAttestorFromProfile(profileId, overrides = {}) {
        ...
        const packId = this.licenseAttestationAnalytics.getPackIdForProfile(result.profile.id)
            || result.profile.metadata?.packId
            || null;
        const binding = {
            ...(result.binding || {}),
        ...
        this.activeLicenseAttestationProfileId = result.profile.id;
        this.licenseAttestationAnalytics.setActiveProfile(result.profile.id);
        this.setLicenseAttestor(result.attestor, binding);
    ...
    recordAudit(event, payload = {}, classification = 'compliance') {
        ...
        this.licenseAttestationAnalytics.recordAuditEntry(entry);
    ...
    getLicenseAttestationAnalyticsSummary() {
        return this.licenseAttestationAnalytics.getSummary();
    }

    getLicenseAttestationAnalyticsReport(options = {}) {
        return this.licenseAttestationAnalytics.generateReport(options);
    }

    exportLicenseAttestationAnalyticsReport(options = {}) {
        return this.licenseAttestationAnalytics.exportReport(options);
    }
    ...
    setLicenseAttestor(attestor, bindingOptions = {}) {
        ...
        if (!this.licenseAttestor) {
            this.activeLicenseAttestationProfileId = null;
            this.licenseAttestationAnalytics.setActiveProfile(null);
            return;
        }

        if (bindingOptions.profileId) {
            const profile = this.licenseAttestationProfiles.getProfile(bindingOptions.profileId);
            const packId = bindingOptions.packId
                || this.licenseAttestationAnalytics.getPackIdForProfile(bindingOptions.profileId)
                || profile?.metadata?.packId
                || null;
            if (profile) {
                this.licenseAttestationAnalytics.registerProfile(profile, { packId });
            }
            this.activeLicenseAttestationProfileId = bindingOptions.profileId;
            this.licenseAttestationAnalytics.setActiveProfile(bindingOptions.profileId);
        } else if (this.activeLicenseAttestationProfileId) {
            this.licenseAttestationAnalytics.setActiveProfile(this.activeLicenseAttestationProfileId);
        } else {
            this.licenseAttestationAnalytics.setActiveProfile(null);
        }
        ...
        if (typeof this.licenseAttestor.on === 'function') {
            const enrichPayload = (payload = {}) => {
                const profileId = this.activeLicenseAttestationProfileId;
                if (!profileId) {
                    return payload || {};
                }
                const profile = this.licenseAttestationProfiles.getProfile(profileId);
                const packId = payload.packId
                    || this.licenseAttestationAnalytics.getPackIdForProfile(profileId)
                    || profile?.metadata?.packId
                    || null;
                const pack = packId ? this.licenseAttestationAnalytics.getPack(packId) : null;
                return {
                    ...payload,
                    profileId: payload.profileId || profileId,
                    profileName: payload.profileName || profile?.name || undefined,
                    packId: packId || undefined,
                    packName: payload.packName || pack?.name || undefined
                };
            };
            ...
            subscribe('attestation', payload => this.recordAudit('compliance.license.attestation', enrichPayload(payload)));
            subscribe('revocation', payload => this.recordAudit('compliance.license.revocation', enrichPayload(payload)));
            subscribe('entitlements', payload => this.recordAudit('compliance.license.entitlements', enrichPayload(payload)));
            subscribe('validation', payload => this.recordAudit('compliance.license.validation', enrichPayload(payload)));
            subscribe('schedule', payload => this.recordAudit('system.license.attestation_scheduled', enrichPayload(payload), 'system'));
            subscribe('error', payload => this.recordAudit('compliance.license.attestor_error', enrichPayload(payload), 'system'));
        }
